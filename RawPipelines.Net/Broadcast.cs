...
namespace RawPipelines.Net
{
    public class Broadcast
    {
        ...
        public event EventHandler<(EndPoint EndPoint, ArraySegment<byte> Buffer)> ReceivedFrom;
        ...
        private async Task RunReceiveFromLoopAsync()
        {
            try
            {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        {
                            var ex = new SocketException((int)awaitable.EventArgs.SocketError);
...
#if NETCOREAPP2_0
                        ReceivedFrom?.Invoke(this, (awaitable.EventArgs.RemoteEndPoint, buffer.Slice(buffer.Offset, bytesReceived)));
#else
                        ReceivedFrom?.Invoke(this, (awaitable.EventArgs.RemoteEndPoint, new ArraySegment<byte>(buffer.Array, buffer.Offset, bytesReceived)));
#endif
                    }
            ...
            catch (Exception ex)
            {
                ...
                else
                {
                    ...
                    // Request UnbindAsync() to be called so we can rethrow this exception, which should be observable.
                    //_appLifetime.StopApplication(); // TODO
                }
            }
        }

        public async Task<SocketError> SendToAsync(ArraySegment<byte> buffer)
        {
